...
void pointwise_conv_sw(const std::vector<std::vector<std::vector<uint8_t>>>& input,
        ...
        for (int i = 0; i < OH; ++i) {
            for (int j = 0; j < OH; ++j) {
                ...
                for (int ic = 0; ic < Cin; ++ic) {
                    int ih = i * stride;
                    int iw = j * stride;
                    int32_t x_int = static_cast<int32_t>(input[ic][ih][iw]) - static_cast<int32_t>(x_zero);
                    int32_t w_int = static_cast<int32_t>(weights[oc][ic][0]);
                    sum += w_int * x_int;
                }
                output[oc][i][j] = static_cast<uint8_t>(
                    std::max(0, std::min(255, static_cast<int>(std::round(sum * (x_scale * w_scale) / y_scale) + y_zero)))
                );
            }
        }
    }
...
void pointwise_conv(const U8* input,
                    const S8* weights,
                    const S32* biases,
                    U8* output,
                    int input_size,
...
void pointwise_conv_wrapper(const std::vector<U8>& input,
    ...
    pointwise_conv_streamed(input.data(), weights.data(), (const S32*)biases.data(), output.data(),
...
int main() {
    ...
    constexpr int OH = (size - 1) / stride + 1;

    std::vector<std::vector<std::vector<uint8_t>>> input(Cin, std::vector<std::vector<uint8_t>>(size, std::vector<uint8_t>(size)));
    std::vector<std::vector<std::vector<int8_t>>> weights(Cout, std::vector<std::vector<int8_t>>(Cin, std::vector<int8_t>(1)));
    ...
    for (int oc = 0; oc < Cout; ++oc) {
        biases[oc] = 1;
        for (int ic = 0; ic < Cin; ++ic)
            weights[oc][ic][0] = (ic - oc + 3) % 5 - 2; // -2 to 2
    }

    // Run reference
    pointwise_conv_sw(input, weights, biases, ref_output, stride, x_zero, y_zero, x_scale, w_scale, y_scale);

    // Flatten inputs for HLS version
    std::vector<U8> flat_input(Cin * size * size);
    std::vector<S8> flat_weights(Cout * Cin);
    ...
    for (int oc = 0; oc < Cout; ++oc)
        for (int ic = 0; ic < Cin; ++ic)
            flat_weights[oc * Cin + ic] = weights[oc][ic][0];

    // Run HLS
    pointwise_conv_wrapper(flat_input, flat_weights, biases, flat_output,
